...
use AiP\Protocol\FCGI\Response;

use AiP\Protocol\FCGI\LogicException;
use AiP\Protocol\FCGI\BadProtocolException;

class FCGI implements \AiP\Protocol
{
    private $stream = null;
    ...
    public function __destruct()
    {
        ...
        // $this->log("DeInitialized FCGI Application: ".get_class($this));
    }
    ...
    public function readRequest($stream, $remote_addr)
    {
        ...
        if (!isset($this->headers['FCGI']) or $this->headers['FCGI'] != '1')
            throw new BadProtocolException("Request is not FCGI/1 Compliant (".var_dump($this->headers, true).")");

        if (!isset($this->headers['CONTENT_LENGTH']))
            throw new BadProtocolException("CONTENT_LENGTH header not present");

        unset($this->headers['FCGI']);
    }
